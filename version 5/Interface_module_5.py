...
from PyQt5.QtCore import *import numpy as np
import pandas as pd
import sys
import pickle
import matplotlib.pyplot as plt
...
from model_module_thread import Model_module
from data_module_thread import Data_module
import time
    ...
    def generate_db(self):
        ...
        for line in range(np.shape(db)[0]):
            ...
            if np.shape(data) == (1, 10, 46):
                ...
                train_untrain_error = model_module.train_untrain_classifier(data=data)
                # normal_abnormal_reconstruction_error = model_module.normal_abnormal_classifier(data=data)
                # abnormal_procedure_prediction, shap_result_positive, shap_result_negative = model_module.abnormal_procedure_classifier_1(data=dim2)
                abnormal_procedure_prediction= model_module.abnormal_procedure_classifier_0(data=dim2)
                diagnosed_scenario = np.argmax(abnormal_procedure_prediction, axis=1)[0]
                shap_result_positive= model_module.abnormal_procedure_classifier_1()
                # shap_result_negative= model_module.abnormal_procedure_classifier_2()
                verif_threshold, abnormal_verif_error = model_module.abnormal_procedure_verification(data=data)
                calculate_time = time.time()
                # print(diagnosed_scenario, abnormal_procedure_prediction[0][diagnosed_scenario]*100)
                ...
                self.shap.emit(shap_result_positive[self.diagnosis_convert_text[diagnosed_scenario]][0])
                self.plot_db.emit([self.liner, self.plot_data], diagnosed_scenario)


                self.display_ex.emit(shap_result_positive[self.diagnosis_convert_text[diagnosed_scenario]][0], [self.liner, self.plot_data], self.compare_data['Normal'])


                # self.another_shap.emit(shap_result_negative, [self.liner, self.plot_data], self.compare_data)
...
class Mainwindow(QWidget):
    ...
    def explain_result(self, shap_result):
        ...
        [self.tableWidget.setItem(i, 0, QTableWidgetItem(f"{shap_result['index'][i]}")) for i in range(len(shap_result['index']))]
    ...
    def __init__(self, loaded_pkl):
        ...
        # 세 번째 그룹 설정
        # gb_3 = QGroupBox(self)
        # gb_3.setStyleSheet("QGroupBox { border: 0.5px solid lightgray;}")
...
class another_result_explain(QWidget):
    ...
    def show_shap(self, symptom_db, compare_data):
        ...
        self.shap_result_negative = model_module.abnormal_procedure_classifier_2(scenario=text_convert_int[self.cb.currentText()[:7]])[self.cb.currentText()[:7]][0]
        # [self.compare_data[compare_data_key].append(self.loaded_pkl[compare_data_key].iloc[symptom_db[0][-1]]) for compare_data_key in self.compare_data.keys()]


        red_range = self.shap_result_negative[self.shap_result_negative['probability'] >= 10]
        orange_range = self.shap_result_negative[
            [self.shap_result_negative['probability'].iloc[i] < 10 and self.shap_result_negative['probability'].iloc[i] > 1 for i in
             range(len(self.shap_result_negative['probability']))]]
        ...
        if self.cb_red1.text().split()[0] != 'None':
            if self.cb_red1.isChecked():
                ...
                self.cb_red_plot_1.plot(x=symptom_db[0], y=pd.DataFrame(compare_data[self.cb.currentText()[:7]])[red_range['index'].iloc[0]], pen=pyqtgraph.mkPen('k', width=3), name=self.cb.currentText()[:7])
        ...
        if self.cb_red2.text().split()[0] != 'None':
            if self.cb_red2.isChecked():
                ...
                self.cb_red_plot_2.plot(x=symptom_db[0], y=pd.DataFrame(compare_data[self.cb.currentText()[:7]])[red_range['index'].iloc[1]], pen=pyqtgraph.mkPen('k', width=3), name=self.cb.currentText()[:7])
        ...
        if self.cb_red3.text().split()[0] != 'None':
            if self.cb_red3.isChecked():
                ...
                self.cb_red_plot_3.plot(x=symptom_db[0], y=pd.DataFrame(compare_data[self.cb.currentText()[:7]])[red_range['index'].iloc[2]], pen=pyqtgraph.mkPen('k', width=3), name=self.cb.currentText()[:7])
        ...
        if self.cb_red4.text().split()[0] != 'None':
            if self.cb_red4.isChecked():
                ...
                self.cb_red_plot_4.plot(x=symptom_db[0], y=pd.DataFrame(compare_data[self.cb.currentText()[:7]])[red_range['index'].iloc[3]], pen=pyqtgraph.mkPen('k', width=3), name=self.cb.currentText()[:7])
        ...
        if self.cb_orange1.text().split()[0] != 'None':
            if self.cb_orange1.isChecked():
                ...
                self.cb_orange_plot_1.plot(x=symptom_db[0], y=pd.DataFrame(compare_data[self.cb.currentText()[:7]])[orange_range['index'].iloc[0]], pen=pyqtgraph.mkPen('k', width=3), name=self.cb.currentText()[:7])
        ...
        if self.cb_orange2.text().split()[0] != 'None':
            if self.cb_orange2.isChecked():
                ...
                self.cb_orange_plot_2.plot(x=symptom_db[0], y=pd.DataFrame(compare_data[self.cb.currentText()[:7]])[orange_range['index'].iloc[1]], pen=pyqtgraph.mkPen('k', width=3), name=self.cb.currentText()[:7])
        ...
        if self.cb_orange3.text().split()[0] != 'None':
            if self.cb_orange3.isChecked():
                ...
                self.cb_orange_plot_3.plot(x=symptom_db[0], y=pd.DataFrame(compare_data[self.cb.currentText()[:7]])[orange_range['index'].iloc[2]], pen=pyqtgraph.mkPen('k', width=3), name=self.cb.currentText()[:7])
        ...
        if self.cb_orange4.text().split()[0] != 'None':
            if self.cb_orange4.isChecked():
                ...
                self.cb_orange_plot_4.plot(x=symptom_db[0], y=pd.DataFrame(compare_data[self.cb.currentText()[:7]])[orange_range['index'].iloc[3]], pen=pyqtgraph.mkPen('k', width=3), name=self.cb.currentText()[:7])
        ...
        if self.cb_orange5.text().split()[0] != 'None':
            if self.cb_orange5.isChecked():
                ...
                self.cb_orange_plot_5.plot(x=symptom_db[0], y=pd.DataFrame(compare_data[self.cb.currentText()[:7]])[orange_range['index'].iloc[4]], pen=pyqtgraph.mkPen('k', width=3), name=self.cb.currentText()[:7])
        ...
        if self.cb_orange6.text().split()[0] != 'None':
            if self.cb_orange6.isChecked():
                ...
                self.cb_orange_plot_6.plot(x=symptom_db[0], y=pd.DataFrame(compare_data[self.cb.currentText()[:7]])[orange_range['index'].iloc[5]], pen=pyqtgraph.mkPen('k', width=3), name=self.cb.currentText()[:7])
        ...
        if self.cb_orange7.text().split()[0] != 'None':
            if self.cb_orange7.isChecked():
                ...
                self.cb_orange_plot_7.plot(x=symptom_db[0], y=pd.DataFrame(compare_data[self.cb.currentText()[:7]])[orange_range['index'].iloc[6]], pen=pyqtgraph.mkPen('k', width=3), name=self.cb.currentText()[:7])
        ...
        if self.cb_orange8.text().split()[0] != 'None':
            if self.cb_orange8.isChecked():
                ...
                self.cb_orange_plot_8.plot(x=symptom_db[0], y=pd.DataFrame(compare_data[self.cb.currentText()[:7]])[orange_range['index'].iloc[7]], pen=pyqtgraph.mkPen('k', width=3), name=self.cb.currentText()[:7])
        ...
        if self.cb_orange9.text().split()[0] != 'None':
            if self.cb_orange9.isChecked():
                ...
                self.cb_orange_plot_9.plot(x=symptom_db[0], y=pd.DataFrame(compare_data[self.cb.currentText()[:7]])[orange_range['index'].iloc[8]], pen=pyqtgraph.mkPen('k', width=3), name=self.cb.currentText()[:7])
        ...
        if self.cb_orange10.text().split()[0] != 'None':
            if self.cb_orange10.isChecked():
                ...
                self.cb_orange_plot_10.plot(x=symptom_db[0], y=pd.DataFrame(compare_data[self.cb.currentText()[:7]])[orange_range['index'].iloc[9]], pen=pyqtgraph.mkPen('k', width=3), name=self.cb.currentText()[:7])
        ...
        if self.cb_orange11.text().split()[0] != 'None':
            if self.cb_orange11.isChecked():
                ...
                self.cb_orange_plot_11.plot(x=symptom_db[0], y=pd.DataFrame(compare_data[self.cb.currentText()[:7]])[orange_range['index'].iloc[10]], pen=pyqtgraph.mkPen('k', width=3), name=self.cb.currentText()[:7])
        ...
        if self.cb_orange12.text().split()[0] != 'None':
            if self.cb_orange12.isChecked():
                ...
                self.cb_orange_plot_12.plot(x=symptom_db[0], y=pd.DataFrame(compare_data[self.cb.currentText()[:7]])[orange_range['index'].iloc[11]], pen=pyqtgraph.mkPen('k', width=3), name=self.cb.currentText()[:7])
    ...
    def show_another_result_table(self, line):
        ...
        self.combo_tableWidget.setRowCount(len(self.shap_result_negative))
        ...
        [self.combo_tableWidget.setItem(i, 0, QTableWidgetItem(f"{self.shap_result_negative['index'][i]}")) for i in
         range(len(self.shap_result_negative['index']))]
        [self.combo_tableWidget.setItem(i, 1, QTableWidgetItem(f"{round(self.shap_result_negative['probability'][i], 2)}%")) for i in
         range(len(self.shap_result_negative['probability']))]
        [self.combo_tableWidget.setItem(i, 2, QTableWidgetItem(f"{self.shap_result_negative['describe'][i]}")) for i in
         range(len(self.shap_result_negative['describe']))]
        # [self.combo_tableWidget.setItem(i, 3, QTableWidgetItem(f"{self.shap_result_negative['system'][i]}")) for i in
        #  range(len(self.shap_result_negative['system']))]
